...
description: 'Cortana 插件开发入口技能。用于统一编排 Native、MCP、Dotnet（已废弃）三类通道，约束工程规范、外部包 AOT 校验、发布安装与运行时更新流程。触发关键词：插件开发、Native 插件、MCP 插件、Dotnet 插件、发布插件、安装插件。'
user-invocable: true
---
...
1. 先判断插件通道：Native、MCP、Dotnet（仅兼容维护）。
2. 先加载 architecture。
3. 需要外部包时加载 packages。
4. Native 场景加载 native；MCP 场景加载 mcp；Dotnet 仅在维护历史插件时加载 dotnet。
5. Native 或 Dotnet 需要发布、安装、更新时加载 publish-install；MCP 场景走连接配置，不走插件包安装。
...
- 新开发优先选择 Native 或 MCP；Dotnet 仅用于历史兼容，不作为新增插件默认方案。
...
- Native AOT：native
- MCP 服务接入：mcp
- Dotnet 托管（废弃，仅兼容维护）：dotnet
- 第三方包：packages
- 发布/安装/热更新（仅 Native/Dotnet）：publish-install
