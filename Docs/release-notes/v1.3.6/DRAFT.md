# Cortana v1.3.6 阶段发布记录（草稿）

> 状态：阶段性草稿，非最终发布文档。  
> 日期：2026-05-10  
> 用途：记录当前阶段已经完成或已合入的修改，后续如继续开发 v1.3.6，可在本文档基础上追加、修订和补充。

## 当前阶段概览

v1.3.6 当前阶段重点围绕 **插件授权、宿主模型能力、长期记忆默认注入、WebSocket 临时系统提示和插件生命周期治理** 展开。

本阶段不是完整发布版定稿，主要记录已经进入代码库的阶段性能力：
...
一句话：**当前阶段把插件从“只能提供工具”推进到“可在授权后调用宿主模型能力和长期记忆控制面”，并补齐了临时系统提示、记忆治理和插件进程清理能力。**
...
当前阶段先提供“大模型授权”配置，支持本地插件和 MCP 服务。
...
当前阶段没有新增独立 HTTP/WS 端点，而是复用 `/internal/conversation-feed/` 长连接承载轻量请求/响应：
...
Native 插件当前由 `Cortana.NativeHost.exe` 子进程加载，每个 Native 插件对应一个宿主子进程。为避免主程序异常退出或托盘退出时留下大量插件进程，本阶段补强子进程清理：
...
本次追加说明用于将 v1.3.6 的阶段性草稿补齐为正式发布候选稿，明确当前阶段的核心价值与发布边界。
...
> 该补充说明仅作为 v1.3.6 正式发布前的准发布文档内容，后续如继续开发可继续在本稿基础上追加、修订和同步到正式 `RELEASE.md`。
...
5. **正式发布文案仍需后续整合**
   - 本文档不是最终 `v1.3.6` Release Notes；
   - 后续新增功能、修复和验证记录需要继续追加到本文档或最终发布文档中。
...
后续继续开发 v1.3.6 时，建议在本文档按以下方式追加：

1. 新增功能写入“核心阶段变更”或新增章节；
2. 修复项单独增加“修复记录”；
3. 不兼容行为增加“迁移提示”；
4. 每次补充验证都追加到“当前验证记录”；
5. 到最终发布前，再整理为正式 `RELEASE.md`。
