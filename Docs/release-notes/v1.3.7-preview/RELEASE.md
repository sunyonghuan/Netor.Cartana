...
v1.3.7 当前重点围绕 **单端点统一插件总线、Memory 插件总线迁移、长期记忆供应链路观测、主窗口状态持久化和版本整理** 展开。
...
7. 补齐 `App.axaml.cs` 中缺失的 XML 注释，便于后续维护。
8. 新增单端点统一插件总线改造计划文档，作为协议演进和验收依据。

一句话：**本预发布版本正在把内部插件通信从多端点、多协议收敛为一个插件总线，同时增强长期记忆链路可观测性，并补齐桌面窗口体验细节。**
...
6. `system.notice` 在 PluginBus envelope 下是否需要统一为 `topic = system` / `op = system.notice`。
7. 旧 release 文档中提到的 `/internal/model-capability/` 和 `/internal/conversation-feed/` 是否需要在正式 v1.3.7 中标注为已废弃。
