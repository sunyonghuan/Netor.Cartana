...
**含 `Main` / `class Program` 的完整控制台程序不是脚本**，不走本技能，交给标准项目工作流（`dotnet new console` + `dotnet run`）。
...
| 脚本已是文件 / 代码 > 256 K 字符 / 需要 `#load` | `csx_run_file`（接受 `.csx` 和顶级语句形式的 `.cs`） |
| 多步任务需要跨调用共享变量、`using`、函数 | `csx_session_create` → `csx_session_exec` × N → `csx_session_close` |
| 只检查能否编译，不执行 | `csx_check` |
| 格式化代码 | `csx_format` |
...
1. 代码是否是脚本形态（见「什么是脚本」）？否 → 不属于本技能
2. `csx_script` 工具是否可用？是 → 用方式 A
3. 否 → 用方式 B
