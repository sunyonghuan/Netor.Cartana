...
Write-Host "`n📊 服务器监控报表 - $(Get-Date -Format 'yyyy-MM-dd HH:mm')" -ForegroundColor Cyan
...
foreach ($server in $servers) {
    ...
    if (Test-Path $keyPath) {
        ...
        try {
            $probe = & ssh -i $keyPath -o BatchMode=yes -o StrictHostKeyChecking=no -o ConnectTimeout=15 -p $server.Port "$username@$ip" "printf 'cpu=NA`nmemory=NA`ndisk=NA`n'" 2>$null
            $status = '✅'
...
$markdown += ''
$markdown += '## 异常告警'
$alerts = $report | Where-Object { $_.Status -eq '❌' }
...
# 显示报表
Write-Host "`n| 服务器 | IP | 状态 | CPU | 内存 | 硬盘 |" -ForegroundColor White
...
# 异常提醒
Write-Host "`n⚠️ 异常提醒：" -ForegroundColor Yellow
$alerts = $report | Where-Object { $_.Status -eq '❌' }
